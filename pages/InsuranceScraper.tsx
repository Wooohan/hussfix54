...
export const InsuranceScraper: React.FC<InsuranceScraperProps> = ({ carriers, onUpdateCarriers, autoStart }) => {
  ...
  const startEnrichmentProcess = async (targetList: CarrierData[] = carriers) => {
    ...
    if (targetList.length === 0) {
        setLogs(prev => [...prev, { text: "Error: No carriers found in range/database.", time: new Date().toLocaleTimeString().split(' ')[0] }]);
        return;
    }
    ...
    setLogs(prev => [...prev, { text: `Insurance enrichment started`, time: new Date().toLocaleTimeString().split(' ')[0] }]);
    setLogs(prev => [...prev, { text: `Targeting: ${targetList.length} USDOT records`, time: new Date().toLocaleTimeString().split(' ')[0] }]);
    ...
    for (let i = 0; i < targetList.length; i++) {
      ...
      setLogs(prev => [...prev, { text: `[INSURANCE] [${i+1}/${targetList.length}] Querying DOT: ${dot}...`, time: new Date().toLocaleTimeString().split(' ')[0] }]);
      ...
      try {
        ...
        if (policies.length > 0) {
          currentInsFound++;
          setLogs(prev => [...prev, { text: `Success: Extracted ${policies.length} insurance filings for ${dot}`, time: new Date().toLocaleTimeString().split(' ')[0] }]);
          ...
          try {
            ...
            if (res) {
              currentDbSaved++;
              setLogs(prev => [...prev, { text: `DB Sync: Record ${dot} updated`, time: new Date().toLocaleTimeString().split(' ')[0] }]);
            }
          } catch (syncErr) {
            setLogs(prev => [...prev, { text: `DB Fail: Could not sync ${dot}`, time: new Date().toLocaleTimeString().split(' ')[0] }]);
          }
        } else {
          setLogs(prev => [...prev, { text: `No active insurance found for ${dot}`, time: new Date().toLocaleTimeString().split(' ')[0] }]);
        }
      } catch (err) {
        currentInsFailed++;
          setLogs(prev => [...prev, { text: `Fail: Insurance timeout for DOT ${dot}`, time: new Date().toLocaleTimeString().split(' ')[0] }]);
      }
    ...
    setLogs(prev => [...prev, { text: `Insurance enrichment complete`, time: new Date().toLocaleTimeString().split(' ')[0] }]);
  };
  ...
  const handleRangeRun = () => {
    ...
    setLogs(prev => [...prev, { text: `Range: MC ${start} to ${end} (${filtered.length} found)`, time: new Date().toLocaleTimeString().split(' ')[0] }]);
          ...
          <div className="flex-1 overflow-y-auto p-10 font-mono text-[11px] space-y-3 custom-scrollbar">
            {logs.map((log, i) => (
              <div key={i} className={`flex gap-4 items-start p-3 rounded-xl transition-all ${
                log.text.includes('Success') || log.text.includes('DB Sync') || log.text.includes('complete') 
                ? 'text-emerald-400 bg-emerald-500/5 border border-emerald-500/10' 
                : log.text.includes('Fail') || log.text.includes('Error') ? 'text-red-400 bg-red-500/5 border border-red-500/10' 
                : 'text-slate-400'
              }`}>
